...
export class CustomerCreateComponent implements OnInit {
  ...
  initForm() {
    ...
    this.personalCustomerForm = new FormGroup({
      ...
      "PersonalPhoneWork": new FormControl(''),
      "PersonalPhoneOther": new FormControl(''),
      "PersonalEmail": new FormControl('', [Validators.pattern(this.emailPattern)]),
      "PersonalEmailWork": new FormControl(''),
      "PersonalEmailOther": new FormControl(''),
    ...
    switch (customerType) {
      case 2:
        ...
        if (!this.personalCustomerForm.valid) {
          ...
          result = await this.customerService.createCustomerAsAccountAsync(companyCustomerModel, companyCustomerContactModel, companyContactList, this.auth.userId, false, false, userModel);
          if (result.statusCode === 200) {
            this.listCustomer = result.listCustomer;
  ...
  mappingPersonalCustomerContacFormToModel(): ContactModel {
    ...
    contactModel.DateOfBirth = this.personalCustomerForm.get('DateOfBirth').value ? convertToUTCTime(this.personalCustomerForm.get('DateOfBirth').value) : null;
    contactModel.CreatedDate = new Date();
    contactModel.Active = true;
