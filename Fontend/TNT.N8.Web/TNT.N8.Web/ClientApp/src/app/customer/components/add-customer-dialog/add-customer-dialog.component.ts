    ...
    switch (customerType) {
      ...
      case 2:
        ...
        if (!this.personalCustomerForm.valid) {
          ...
          contactModelOnSave.WorkEmail = this.personalCustomerForm.get('WorkEmail').value;
          //chỉ check khi chưa đổ data
          ...
          let employees = new CreateEmployeeModel();
          employees.DanToc = this.personalCustomerForm.get('DanToc').value;
          employees.QuocTich = this.personalCustomerForm.get('QuocTich').value;
          employees.TonGiao = this.personalCustomerForm.get('TonGiao').value;
          employees.EmployeeCode = this.personalCustomerForm.get('EmployeeCode').value;
          let userModel = new UserModel();
          userModel.EmployeeCode = this.personalCustomerForm.get('EmployeeCode').value;
          let result: any
          let isEmployee = this.checked
          if (this.checked) {
            // result = await this.customerService.createCustomerAsEmployeeAsync(companyCustomerModel, companyCustomerContactModel, companyContactList, this.auth.userId, false, false, listPhongBanId, employees, userModel,isEmployee);
          } else {
            result = await this.customerService.createCustomerAsync(companyCustomerModel, companyCustomerContactModel, companyContactList, this.auth.userId, false, false);
          }
          if (result.statusCode === 200) {
            this.listCustomer = result.listCustomer;
            this.sendEmail(result);
    ...
    ref.onClose.subscribe((result: any) => {
      if (result) {
        if (result?.length > 0) {
          ...
          this.personalCustomerForm.controls.OrganizationId.patchValue(listSelectedTenDonVi);
        }
        else {
          this.listSelectedDonVi = [];
          this.personalCustomerForm.controls.OrganizationId.patchValue(null);
        }
      }
    });
  }
  showFormCreateEmployee(e){
